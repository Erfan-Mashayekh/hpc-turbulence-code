...
UniformMeshsize::UniformMeshsize(const Parameters& parameters)
    ...
    if (dx_ <= 0.0) { HANDLE_ERROR(1, "dx_ <= 0.0!"); }
    if (dy_ <= 0.0) { HANDLE_ERROR(1, "dy_ <= 0.0!"); }
    if (parameters.geometry.dim == 3) {
        if (dz_ <= 0.0) { HANDLE_ERROR(1, "dz_ <= 0.0!"); }
    }
}
...
TanhMeshStretching::TanhMeshStretching(const Parameters& parameters, bool stretchX, bool stretchY, bool stretchZ)
    ...
    , dxMin_(stretchX ? 0.5 * parameters.geometry.lengthX * (1.0 + tanh(deltaS_ * (2.0 / sizeX_ - 1.0)) / tanhDeltaS_) : uniformMeshsize_.getDx(0, 0))
    , dyMin_(stretchY ? 0.5 * parameters.geometry.lengthY * (1.0 + tanh(deltaS_ * (2.0 / sizeY_ - 1.0)) / tanhDeltaS_) : uniformMeshsize_.getDy(0, 0))
    , dzMin_(stretchZ ? 0.5 * parameters.geometry.lengthZ * (1.0 + tanh(deltaS_ * (2.0 / sizeZ_ - 1.0)) / tanhDeltaS_) : uniformMeshsize_.getDz(0, 0, 0)) {}
