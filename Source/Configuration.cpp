...
void readFloatMandatory(FLOAT & storage, tinyxml2::XMLElement *node, const char* tag) {
    ...
    if (node->QueryDoubleAttribute(tag, &value) != tinyxml2::XML_NO_ERROR) {
        HANDLE_ERROR(1, "Error while reading mandatory argument");
    } else {
        storage = (FLOAT)value;
    }
}

void readFloatOptional(FLOAT & storage, tinyxml2::XMLElement *node, const char* tag, FLOAT defaultValue = 0) {
    double value; // Use to be able to select precision
    ...
    if (result == tinyxml2::XML_NO_ATTRIBUTE){
        ...
        HANDLE_ERROR(1, "Error while reading optional argument");
    } else {
        storage = (FLOAT)value;
    }
}
...
void readStringMandatory(std::string & storage, tinyxml2::XMLElement *node) {
    ...
    if (myText == NULL) {
        ...
        if (!storage.compare("")) {
            HANDLE_ERROR(1, "Missing mandatory string!");
        }
    }
}
...
void Configuration::loadParameters(Parameters & parameters, const MPI_Comm & communicator) {
    ...
    if (rank == 0) {
        ...
        if (confFile.FirstChildElement() == NULL) {
            HANDLE_ERROR(1, "Error parsing the configuration file");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading geometry properties");
        }
        ...
        if (parameters.geometry.sizeX < 2 || parameters.geometry.sizeY < 2 || parameters.geometry.sizeZ < 0) {
            HANDLE_ERROR(1, "Invalid size specified in configuration file");
        }
        ...
        if (parameters.geometry.dim == 3 && parameters.geometry.sizeZ == 1) {
            HANDLE_ERROR(1, "Inconsistent data: 3D geometry specified with Z size zero");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading timestep parameters");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading flow parameters");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading solver parameters");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading environmental  parameters");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading simulation parameters");
        }
        ...
        if (subNode != NULL) {
            ...
            HANDLE_ERROR(1, "Missing type in simulation parameters");
        }
	...
	//--------------------------------------------------
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading VTK parameters");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading StdOut parameters");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading parallel parameters");
        }
        ...
        if (node == NULL) {
            HANDLE_ERROR(1, "Error loading wall parameters");
        }
