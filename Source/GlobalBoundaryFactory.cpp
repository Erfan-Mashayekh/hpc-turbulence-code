...
GlobalBoundaryFactory::GlobalBoundaryFactory(Parameters& parameters)
    ...
    if (scenario == "cavity") {
        ...
        HANDLE_ERROR(1, "Scenario not recognized");
    }
}
...
GlobalBoundaryIterator<FlowField> GlobalBoundaryFactory::getGlobalBoundaryFGHIterator(FlowField& flowField) {
    if (parameters_.geometry.dim == 2) {
        return GlobalBoundaryIterator<FlowField>(flowField, parameters_,
            *(FGHStencils_[0]), *(FGHStencils_[1]),
            *(FGHStencils_[2]), *(FGHStencils_[3]),
            1, 0);
    }
    return GlobalBoundaryIterator<FlowField>(flowField, parameters_,
        *(FGHStencils_[0]), *(FGHStencils_[1]),
        *(FGHStencils_[2]), *(FGHStencils_[3]),
        *(FGHStencils_[4]), *(FGHStencils_[5]),
        1, 0);
}
...
GlobalBoundaryIterator<FlowField> GlobalBoundaryFactory::getGlobalBoundaryVelocityIterator(FlowField& flowField) {
    if (parameters_.geometry.dim == 2) {
        return GlobalBoundaryIterator<FlowField>(flowField, parameters_,
            *(velocityStencils_[0]), *(velocityStencils_[1]),
            *(velocityStencils_[2]), *(velocityStencils_[3]),
            1, 0);
    }
    return GlobalBoundaryIterator<FlowField>(flowField, parameters_,
            *(velocityStencils_[0]), *(velocityStencils_[1]),
            *(velocityStencils_[2]), *(velocityStencils_[3]),
            *(velocityStencils_[4]), *(velocityStencils_[5]),
            1, 0);
}
