...
void VelocityStencil::apply(FlowField& flowField, int i, int j) {
    ...
    if ((obstacle & OBSTACLE_SELF) == 0){ // If this is a fluid cell
        if ((obstacle & OBSTACLE_RIGHT) == 0){  // Check whether the neighbor is also fluid
            ...
            const FLOAT dx = 0.5 * (parameters_.meshsize->getDx(i, j) + parameters_.meshsize->getDx(i + 1, j));
            velocity.getVector(i, j)[0] = flowField.getFGH().getVector(i, j)[0] - dt / dx * (flowField.getPressure().getScalar(i + 1, j) - flowField.getPressure().getScalar(i, j));
        ...
        if ((obstacle & OBSTACLE_TOP) == 0) {
            const FLOAT dy = 0.5 * (parameters_.meshsize->getDy(i, j) + parameters_.meshsize->getDy(i, j + 1));
            velocity.getVector(i, j)[1] = flowField.getFGH().getVector(i, j)[1] - dt / dy * (flowField.getPressure().getScalar(i, j + 1) - flowField.getPressure().getScalar(i, j));
        } else {
...
void VelocityStencil::apply(FlowField& flowField, int i, int j, int k) {
    ...
    if ((obstacle & OBSTACLE_SELF) == 0) {
        if ((obstacle & OBSTACLE_RIGHT) == 0) {
            ...
            velocity.getVector(i, j, k)[0] = flowField.getFGH().getVector(i, j, k)[0] - dt / dx * (flowField.getPressure().getScalar(i + 1, j, k) - flowField.getPressure().getScalar(i, j, k));
        } else {
            velocity.getVector(i, j, k)[0] = 0.0;
        }
        if ((obstacle & OBSTACLE_TOP) == 0) {
            const FLOAT dy = 0.5 * (parameters_.meshsize->getDy(i, j, k) + parameters_.meshsize->getDy(i, j + 1, k));
            velocity.getVector(i, j, k)[1] = flowField.getFGH().getVector(i, j, k)[1] - dt / dy * (flowField.getPressure().getScalar(i, j + 1, k) - flowField.getPressure().getScalar(i, j, k));
        } else {
        ...
        if ((obstacle & OBSTACLE_BACK) == 0) {
            ...
            velocity.getVector(i, j, k)[2] = flowField.getFGH().getVector(i, j, k)[2] - dt / dz * (flowField.getPressure().getScalar(i, j, k + 1) - flowField.getPressure().getScalar(i, j, k));
        } else {
